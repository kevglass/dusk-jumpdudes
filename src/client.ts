...
class JumpDudesGame {
  ...
  constructor() {
    ...
    if (!touchDevice) {
      ;(document.getElementById("jump") as HTMLImageElement).addEventListener(
        "mousedown",
        () => {
          this.jump()
        }
    ...
    ;(document.getElementById("left") as HTMLDivElement).addEventListener(
      "click",
      () => {
    ...
    ;(document.getElementById("right") as HTMLDivElement).addEventListener(
      "click",
      () => {
        if (this.atPlayerSelect) {
          this.selectTarget++
    ...
    ;(document.getElementById("join") as HTMLDivElement).addEventListener(
      "click",
      () => {
        if (this.atPlayerSelect) {
          ...
          play(SOUND_START)
          ;(
            document.getElementById("startScreen") as HTMLDivElement
          ).style.display = "none"
  ...
  updateTarget() {
    ...
    this.playerSelectionWheel.rotation.y =
      (-this.currentTarget * Math.PI * 2) / this.playerModels.length
    if (this.currentTarget === this.selectTarget) {
      ;(document.getElementById("name") as HTMLDivElement).innerHTML =
        PLAYER_TYPES[this.getSelectedPlayerType()].name
    } else {
      ;(document.getElementById("name") as HTMLDivElement).innerHTML = ""
    }
  }
  ...
  getSelectedPlayerType(): number {
    return (
      (this.selectTarget < 0
        ? PLAYER_TYPES.length + (this.selectTarget % PLAYER_TYPES.length)
        : this.selectTarget) % PLAYER_TYPES.length
    )
  }
    ...
    Dusk.initClient({
      onChange: ({ game, yourPlayerId }) => {
        ...
        for (const body of game.world.bodies) {
          ...
          if (!objBounds) {
            const geometry =
              body.type === ShapeType.CYLINDER
                ? new CylinderGeometry(
                    body.size.x / 2,
                    body.size.x / 2,
                    body.size.y
                  )
                : new BoxGeometry(body.size.x, body.size.y, body.size.z)
            const material = new MeshBasicMaterial({
              color: 0xff0000,
              wireframe: true,
        ...
        for (const p of game.players) {
          ...
          if (body) {
            this.players[p.id].position.set(
              body.center.x,
              body.center.y - PLAYER_HEIGHT / 2,
              body.center.z
            )
  ...
  render(): void {
    if (this.atPlayerSelect) {
      ;(document.getElementById("jump") as HTMLImageElement).style.display =
        "none"
      ;(document.getElementById("joystick") as HTMLImageElement).style.display =
        "none"
  ...
  sendControls(): void {
    if (
      Date.now() - this.lastActionSent > SEND_ACTION_INTERVAL &&
      this.localPlayerId
    ) {
      if (
        this.lastSentControls.x !== this.controls.x ||
        this.lastSentControls.y !== this.controls.y ||
        this.lastSentControls.jump !== this.controls.jump
      ) {
  ...
  renderScene(): void {
    ...
    if (this.playerObject) {
      ...
      this.camera.position.set(
        this.playerObject.position.x + this.viewDistance * -direction.x,
        y + h,
        this.playerObject.position.z + this.viewDistance * -direction.z
      )
      this.camera.lookAt(
        this.playerObject.position.x + frontDistance * direction.x,
        y,
        this.playerObject.position.z + frontDistance * direction.z
      )
      ...
      const unhide: Mesh[] = this.hidden.filter(
        (h) => !hiddenThisFrame.includes(h)
      )
