...
export function rotateVec2(v: Vec2, center: Vec2, angle: number): Vec2 {
    ...
    return { x: x * Math.cos(angle) - z * Math.sin(angle) + center.x, z: x * Math.sin(angle) + z * Math.cos(angle) + center.z }
}
...
function testCollision(world: World, c1: Body, c2: Body, collisionInfo: CollisionDetails): boolean {
    ...
    if (c1.type == ShapeType.CYLINDER && c2.type === ShapeType.CYLINDER) {
        const
            vFrom1to2 = subVec2(c2.center, { x: c1.center.x, z: c1.center.z }),
            ...
            if (status2) {
                ...
                if (collisionInfoR1.depth < collisionInfoR2.depth) {
                    setCollisionInfo(collisionInfo, collisionInfoR1.depth, collisionInfoR1.normal,
                        subVec2(collisionInfoR1.start, scaleVec2(collisionInfoR1.normal, collisionInfoR1.depth)));
                    return true;
                }
                else {
    ...
    if (c1.type === ShapeType.BOX && c2.type === ShapeType.CYLINDER) {
        let inside = 1,
            ...
            nearestEdge = 0,
            i, v,
            circ2Pos: Vec2 | undefined, projection;
        for (i = 4; i--;) {
        ...
        if (inside && circ2Pos) {
            ...
            let
                v1 = subVec2(circ2Pos, c1.vertices[nearestEdge]),
                v2 = subVec2(c1.vertices[(nearestEdge + 1) % 4], c1.vertices[nearestEdge]),
...
function setCollisionInfo(collision: CollisionDetails, D: number, N: Vec2, S: Vec2) {
...
function findAxisLeastPenetration(rect: Body, otherRect: Body, collisionInfo: CollisionDetails) {
    ...
    if (hasSupport) {
        ...
        setCollisionInfo(collisionInfo, bestDistance, rect.faceNormals[bestIndex], addVec2(supportPoint as Vec2, scaleVec2(rect.faceNormals[bestIndex], bestDistance)));
    }
...
function computeRectNormals(vertices: Vec2[]): Vec2[] {
    ...
    for (let i = 4; i--;) {
        faceNormals[i] = normalize(subVec2(vertices[(i + 1) % 4], vertices[(i + 2) % 4]));
    }
